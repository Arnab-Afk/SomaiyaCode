...
const Bann = () => {
  ...
  const handleClick = async () => {
    ...
    try {
      const response = await axios.post("https://resume-screening-3.onrender.com/inter", {
        prompt: userData,
      });
    ...
    <>
      ...
      >
        <div className="flex flex-col ml-32 mr-32 w-1/2 items-start gap-3 justify-center h-full text-center">
          ...
          <button
            onClick={handleClick}
            className="outline-none bg-transparent border-white p-3 pl-8 pr-8 border-2 rounded-md transition duration-500 hover:bg-white hover:text-black"
          >
